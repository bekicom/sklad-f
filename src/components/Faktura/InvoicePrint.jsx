...
const InvoicePrint = forwardRef(({ sale = {}, onPrintStart }, ref) => {
  ...
  // 1️⃣ Joriy to‘lov va summalar
  const totalAmount = Number(
    sale.total_amount || sale.total || sale.finalAmount || 0
  );
  const paidAmount = Number(sale.paid_amount || 0); // endi har doim haqiqiy to‘lov
  ...
  // Productlarga tegishli oldingi qarzlar
  ...
  const previousDebtEffective = (() => {
    ...
    const currentDebt = Math.max(finalAmount - paidAmount, 0);

    return Math.max(custTotalDebt - currentDebt, 0);
  })();
